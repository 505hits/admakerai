...
export default function LoginPagePt() {
    return (
        <Suspense fallback={<div className={styles.authContainer}><div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', color: 'white' }}>Carregando...</div></div>}>
            <LoginContent />
        </Suspense>
    );
}
...
function LoginContent() {
    ...
    useEffect(() => {
        ...
        if (errorParam) {
            setError(`Falha na autenticação: ${errorParam}. Por favor, tente novamente.`);
            setLoading(false);
        }
    }, [searchParams]);
        ...
        const handleCodeExchange = async () => {
            ...
            if (code) {
                try {
                    ...
                    if (data.session) {
                        window.history.replaceState(null, '', '/pt/conexao');
    ...
    const handleGoogleLogin = async () => {
        try {
            ...
            setError('Falha ao entrar com Google. Por favor, tente novamente.');
            setLoading(false);
        }
    };
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Falha ao enviar o link mágico. Por favor, verifique seu email e tente novamente.');
        } finally {
            setLoading(false);
        }
    };
