...
export default function LoginPageFr() {
    return (
        <Suspense fallback={<div className={styles.authContainer}><div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', color: 'white' }}>Chargement...</div></div>}>
            <LoginContent />
        </Suspense>
    );
}
...
function LoginContent() {
    ...
    useEffect(() => {
        ...
        if (errorParam) {
            setError(`Échec de l'authentification : ${errorParam}. Veuillez réessayer.`);
            setLoading(false);
        }
    }, [searchParams]);
        ...
        const handleCodeExchange = async () => {
            ...
            if (code) {
                try {
                    ...
                    if (data.session) {
                        window.history.replaceState(null, '', '/fr/connexion');
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Échec de l\'envoi du lien magique. Veuillez vérifier votre email et réessayer.');
        } finally {
            setLoading(false);
        }
    };
