...
export default function LoginPageEs() {
    return (
        <Suspense fallback={<div className={styles.authContainer}><div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', height: '100vh', color: 'white' }}>Cargando...</div></div>}>
            <LoginContent />
        </Suspense>
    );
}
...
function LoginContent() {
    ...
    useEffect(() => {
        ...
        if (errorParam) {
            setError(`Error de autenticación: ${errorParam}. Por favor, inténtelo de nuevo.`);
            setLoading(false);
        }
    }, [searchParams]);
        ...
        const handleCodeExchange = async () => {
            ...
            if (code) {
                try {
                    ...
                    if (data.session) {
                        window.history.replaceState(null, '', '/es/iniciar-sesion');
                        router.push('/es/pago');
                    }
                } catch (err) {
                    setError('Error de autenticación. Por favor, inténtelo de nuevo.');
    ...
    const handleEmailSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            setError('Error al enviar el enlace mágico. Por favor, verifique su email e inténtelo de nuevo.');
        } finally {
            setLoading(false);
        }
    };
