...
let content = fs.readFileSync('app/fr/tableau-de-bord/page.tsx', 'utf8');
...
for (const [english, french] of Object.entries(translations)) {
    ...
    content = content.replace(new RegExp(`>${escapedEnglish}<`, 'g'), `>${french}<`);
    content = content.replace(new RegExp(`"${escapedEnglish}"`, 'g'), `"${french}"`);
    content = content.replace(new RegExp(`'${escapedEnglish}'`, 'g'), `'${french}'`);
}

// Write back
fs.writeFileSync('app/fr/tableau-de-bord/page.tsx', content, 'utf8');

console.log("Dashboard translation completed!");
